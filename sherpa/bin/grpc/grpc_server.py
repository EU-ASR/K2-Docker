...
class StreamingServer(audio_to_text_pb2_grpc.Module_ASRCoreServicer):
    def __init__(
        self,
    ...
    async def stream_consumer_task(self):
        ...
        while True:
            try:
                ...
                logging.exception("Unhandled exception in stream consumer task")
    ...
    async def TranscribeBuffer(self, request_iterator: AsyncIterable[audio_to_text_pb2.AudioBuffer], 
        ...
        """
        The core method of ASRCore ... handling the audio streams
        """
        try:
            if self.current_active_connections < self.max_active_connections:
                ...
                response = "The server is busy. Please retry later."
                context.abort(grpc.StatusCode.RESOURCE_EXHAUSTED, response)

            async for response in self._transcribe_stream(request_iterator):
                yield response
        except:
            logging.exception(f"{context.peer()} disconnected")
        finally:
            # Decrement so that it can accept new connections
            self.current_active_connections -= 1

            logging.info(
                f"Disconnected: {context.peer()}. "
                f"Number of connections: {self.current_active_connections}/{self.max_active_connections}"  # noqa
            )
    

    def _pack_rpc_message(self, message) -> audio_to_text_pb2.RecognizedTokens:
        """
        Create RecognizedTokens structure from Sherpa result
        """     
        recognized_sentence = audio_to_text_pb2.RecognizedTokens()
        for word in message["text"].split():
            recognized_sentence.tokens.append(
                audio_to_text_pb2.Token(token=word, likelihood=float(0)))

        return recognized_sentence

    async def _transcribe_stream(
        self,
        request_iterator: AsyncIterable[audio_to_text_pb2.AudioBuffer]
    ) -> AsyncIterable[audio_to_text_pb2.RecognizedTokens]:
        ...
        async for samples in self._recv_audio_samples(request_iterator):
            if samples is None:
                break
            ...
            while self.recognizer.is_ready(stream):
                ...
                yield self._pack_rpc_message(message)
    ...
    async def _recv_audio_samples(
        self,
        request_iterator: AsyncIterable[audio_to_text_pb2.AudioBuffer]
    ) -> AsyncIterable[Optional[torch.Tensor]]:
        """Receives a tensor from the client.

        Each message contains either a bytes buffer containing audio samples
        in float32 or contains "Done" meaning the end of utterance.
        ...
        async for buffer in request_iterator:
            np_buffer = np.empty((len(buffer.samples)), dtype=np.float32)
            for index, sample in enumerate(buffer.samples):
                np_buffer[index] = sample.sample
            yield torch.from_numpy(np_buffer)
